...
void NavPointGenerator::generate(MeshInstance3D* _mesh) {
    ...
    Transform3D world_space = _mesh->get_global_transform();
    Ref<Mesh> mesh = _mesh->get_mesh();
    
    print_line("surface count: ", mesh->get_surface_count());
    ...
    for (int s = 0; s < mesh->get_surface_count(); s++) {
        ...
        print_line("indice count: ", indices.size());
        print_line("vertice count: ", vertices.size());
